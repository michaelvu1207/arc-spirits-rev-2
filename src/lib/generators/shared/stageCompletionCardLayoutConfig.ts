/**
 * Shared types and helpers for the Stage Completion Card layout system.
...
export type StageCompletionPlacementKey = 'title' | 'complete_condition' | 'stage_text' | 'advance_to_next_stage_label';

export type StageCompletionCardLayoutConfig = {
	_version: 1;
	_ref_w: number;
	_ref_h: number;
	placements: Record<StageCompletionPlacementKey, Transform>;
	text_boxes: Record<StageCompletionPlacementKey, Box>;
	text_styles: Record<StageCompletionPlacementKey, TextStyle>;
	defaultBaseImagePath?: string | null;
};

export const PLACEMENT_KEYS: { key: StageCompletionPlacementKey; label: string }[] = [
	{ key: 'title', label: 'Title' },
	{ key: 'complete_condition', label: 'Complete Condition' },
	{ key: 'stage_text', label: 'Stage Text' },
	{ key: 'advance_to_next_stage_label', label: 'To advance to the next stage:' }
];

export function createDefaultConfig(refW: number, refH: number): StageCompletionCardLayoutConfig {
	return {
		_version: 1,
		_ref_w: refW,
		_ref_h: refH,
		placements: {
			...
			complete_condition: { x: refW / 2, y: 200, rotation: 0, scale: 1 },
			stage_text: { x: refW / 2, y: 260, rotation: 0, scale: 1 },
			advance_to_next_stage_label: { x: refW / 2, y: 320, rotation: 0, scale: 1 }
		},
		text_boxes: {
			title: { w: Math.round(refW * 0.8), h: 60 },
			complete_condition: { w: Math.round(refW * 0.75), h: 160 },
			stage_text: { w: Math.round(refW * 0.75), h: 60 },
			advance_to_next_stage_label: { w: Math.round(refW * 0.75), h: 60 }
		},
		text_styles: {
			title: { fontSize: 28, lineHeight: 32, align: 'center', color: '#ffffff', fontFamily: 'Opsilon' },
			complete_condition: { fontSize: 16, lineHeight: 20, align: 'center', color: '#fbbf24' },
			stage_text: { fontSize: 26, lineHeight: 30, align: 'center', color: '#ffffff' },
			advance_to_next_stage_label: { fontSize: 16, lineHeight: 20, align: 'center', color: '#fbbf24' }
		}
	};
}
...
export function coerceConfig(raw: unknown, fallbackW?: number, fallbackH?: number): StageCompletionCardLayoutConfig | null {
